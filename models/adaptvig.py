...
class AdaptViG(torch.nn.Module):
    def __init__(self, blocks, channels, kernels, stride,
        ...
        self.prediction = nn.Sequential(
            ...
            nn.Conv2d(channels[-1], emb_dims, 1, bias=True),
    ...
    def forward(self, inputs):
        x = self.stem(inputs)
        for stage_block in self.backbone:
            x = stage_block(x)
...
@register_model
def AdaptViG_S(pretrained=False, **kwargs):
    model = AdaptViG(
...
@register_model
def AdaptViG_M(pretrained=False, **kwargs):
    model = AdaptViG(
        blocks=[[4,4], [4,4], [12,4], [4,4]],
...
@register_model
def AdaptViG_B(pretrained=False, **kwargs):
    model = AdaptViG(
        blocks=[[5,5], [5,5], [15,5], [5,5]],
